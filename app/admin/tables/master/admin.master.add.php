...
if(isset($_POST["addMaster"])){
    ...
    $add = Master::addMaster($_POST, $newName);
    if(!empty($add)){
        $idMaster = Master::findMaster($_POST["name"],$_POST["surname"]);
        $typiesId=$_POST["typeOfService_id"];
        for ($i = 0; $i<=count($typiesId);$i++){
            Master::addMasterService($idMaster->id,$typiesId[$i]);
        }
        header("Location: \app\admin/tables/master/admin.master.php");
    }
}else {
    header("Location: /app/admin/tables/master/admin.master.add.php");
}
}

include $_SERVER["DOCUMENT_ROOT"]."/app/admin/views/admin.master.add.view.php";